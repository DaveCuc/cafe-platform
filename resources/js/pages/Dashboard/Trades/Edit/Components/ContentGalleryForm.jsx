            ...
            {isEditing && (
                <form onSubmit={onUpload} className="mt-4 space-y-4">
                    <CustomFileUpload
                        ...
                        isLoading={isLoading}
                    />
                    <Button disabled={!files.length || isLoading || !remainingSlots} type="submit" className="w-full md:w-auto rounded-none bg-brand text-white hover:bg-brand-darker font-bold uppercase tracking-wider">
                        Subir y Guardar{files.length > 1 ? "s" : ""}
                    </Button>
                </form>
            )}
        </div>
